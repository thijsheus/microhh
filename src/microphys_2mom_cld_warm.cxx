...
namespace mp3d
{
    ...
    void autoconversion(TF* const restrict qrt, TF* const restrict nrt,
                        ...
                        const TF* const restrict rho, const TF* const restrict exner, const TF* const restrict nc,
                        const int istart, const int jstart, const int kstart,
                        const int iend,   const int jend,   const int kend,
                        const int jj, const int kk)
    {
...
template<typename TF>
Microphys_2mom_cld_warm<TF>::Microphys_2mom_cld_warm(Master& masterin, Grid<TF>& gridin, Fields<TF>& fieldsin, Input& inputin) :
    Microphys<TF>(masterin, gridin, fieldsin, inputin)
{
    ...
    fields.init_prognostic_field("nc", "Number density cloud", "m-3", group_name, gd.sloc, false);
    ...
    fields.sp.at("nc")->visc = inputin.get_item<TF>("fields", "svisc", "nc");
}


template<typename TF>
Microphys_2mom_cld_warm<TF>::~Microphys_2mom_cld_warm()
{
}

template<typename TF>
void Microphys_2mom_cld_warm<TF>::init()
{
    auto& gd = grid.get_grid_data();
...
template<typename TF>
void Microphys_2mom_cld_warm<TF>::create(
        ...
        Stats<TF>& stats, Cross<TF>& cross, Dump<TF>& dump, Column<TF>& column)
{
    ...
    if (stats.get_switch())
    {
        ...
        stats.add_tendency(*fields.st.at("nc") , "z", tend_name, tend_longname);
    }
...
void Microphys_2mom_cld_warm<TF>::exec(Thermo<TF>& thermo, const double dt, Stats<TF>& stats)
{
    ...
    remove_negative_values(fields.sp.at("nr")->fld.data(), gd.istart, gd.jstart, gd.kstart,
                           gd.iend, gd.jend, gd.kend, gd.icells, gd.ijcells);
    remove_negative_values(fields.sp.at("nc")->fld.data(), gd.istart, gd.jstart, gd.kstart,
                           gd.iend, gd.jend, gd.kend, gd.icells, gd.ijcells);
    ...
    mp3d::autoconversion(fields.st.at("qr")->fld.data(), fields.st.at("nr")->fld.data(), fields.st.at("qt")->fld.data(), fields.st.at("thl")->fld.data(),
                         fields.sp.at("qr")->fld.data(), ql->fld.data(), fields.rhoref.data(), exner.data(), fields.sp.at("nc")->fld.data(),
    ...
    stats.calc_tend(*fields.st.at("nc"),  tend_name);
}
#endif

template<typename TF>
void Microphys_2mom_cld_warm<TF>::exec_stats(Stats<TF>& stats, Thermo<TF>& thermo, const double dt)
{
    auto& gd = grid.get_grid_data();
...
template<typename TF>
void Microphys_2mom_cld_warm<TF>::exec_cross(Cross<TF>& cross, unsigned long iotime)
{
    TF no_offset = 0.;
    if (cross.get_switch())
    {
...
template<typename TF>
bool Microphys_2mom_cld_warm<TF>::has_mask(std::string name)
{
...
template<typename TF>
void Microphys_2mom_cld_warm<TF>::get_mask(Stats<TF>& stats, std::string mask_name)
{
    auto& gd = grid.get_grid_data();
...
template<typename TF>
void Microphys_2mom_cld_warm<TF>::get_surface_rain_rate(std::vector<TF>& field)
{
...
template <typename TF>
void Microphys_2mom_cld_warm<TF>::update_time_dependent(Timeloop<TF>& timeloop)
{
