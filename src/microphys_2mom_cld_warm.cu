...
using namespace Micro_2mom_cld_warm_constants;
using namespace Micro_2mom_cld_warm_functions;
...
namespace micro
{
    ...
    void autoconversion_g(TF* const __restrict__ qrt, TF* const __restrict__ nrt,
                          TF* const __restrict__ qtt, TF* const __restrict__ nct, TF* const __restrict__ thlt,
                          const TF* const __restrict__ qr,  const TF* const __restrict__ ql,
                          const TF* const __restrict__ rho, const TF* const __restrict__ exner, const TF* const __restrict__ nc,
                          ...
                          const int jj, const int kk)
    {
        ...
        if (i < iend && j < jend && k < kend)
        {
            ...
            if (ql[ijk] > ql_min<TF>)
            {
                const TF xc      = rho[k] * ql[ijk] / nc[ijk];    // Mean mass of cloud drops [kg]
    ...
    template<typename TF> __global__
    void accretion_g(TF* const __restrict__ qrt, TF* const __restrict__ qtt, TF* const __restrict__ nct, TF* const __restrict__ thlt,
                     const TF* const __restrict__ qr,  const TF* const __restrict__ ql,  const TF* const __restrict__ nc,
    ...
    void evaporation_g(TF* const restrict qrt, TF* const restrict nrt,
                       TF* const restrict qtt, TF* const restrict nct, TF* const restrict thlt,
                       ...
                       const int jj, const int kk)
    {
        ...
        if (i < iend && j < jend && k < kend)
        {
            ...
            if (qr[ijk] > qr_min<TF>)
            {
                ...
                qtt[ijk]  -= ev_tend;
                // nct[ijk]  -= lambda_evap * ev_tend * rho[k] / mr;
...
void Microphys_2mom_cld_warm<TF>::exec(Thermo<TF>& thermo, const double dt, Stats<TF>& stats)
{
    ...
    // Remove negative values from the qr, nc, and nr fields
    ...
    micro::remove_negative_values_g<TF><<<gridGPU, blockGPU>>>(fields.ap.at("nc")->fld_g,
        gd.istart, gd.jstart, gd.kstart, gd.iend, gd.jend, gd.kend, gd.icells, gd.ijcells);
    cuda_check_error();
    ...
    micro::autoconversion_g<TF><<<gridGPU, blockGPU>>>(
        ...
        fields.st.at("qt")->fld_g, fields.st.at("nc")->fld_g, fields.st.at("thl")->fld_g,
        fields.sp.at("qr")->fld_g, ql->fld_g, fields.rhoref_g, exner, fields.sp.at("nc")->fld_g,
    ...
    micro::accretion_g<TF><<<gridGPU, blockGPU>>>(
        fields.st.at("qr")->fld_g, fields.st.at("qt")->fld_g, fields.st.at("nc")->fld_g, fields.st.at("thl")->fld_g,
        fields.sp.at("qr")->fld_g, ql->fld_g, fields.sp.at("nc")->fld_g, fields.rhoref_g, exner,
        gd.istart, gd.jstart, gd.kstart,
        gd.iend,   gd.jend,   gd.kend,
        gd.icells, gd.ijcells);
    cuda_check_error();
    ...
    micro::evaporation_g<TF><<<gridGPU, blockGPU>>>(
        fields.st.at("qr")->fld_g, fields.st.at("nr")->fld_g,  fields.st.at("qt")->fld_g,  fields.st.at("nc")->fld_g, fields.st.at("thl")->fld_g,
...
template<typename TF>
unsigned long Microphys_2mom_cld_warm<TF>::get_time_limit(unsigned long idt, const double dt)
{
    auto& gd = grid.get_grid_data();
...
template<typename TF>
void Microphys_2mom_cld_warm<TF>::get_surface_rain_rate_g(
    TF* const __restrict__ rr_out)
{
    auto& gd = grid.get_grid_data();
...
template<typename TF>
void Microphys_2mom_cld_warm<TF>::exec_column(Column<TF>& column)
{
...
template<typename TF>
void Microphys_2mom_cld_warm<TF>::prepare_device()
{
    auto& gd = grid.get_grid_data();
...
template<typename TF>
void Microphys_2mom_cld_warm<TF>::backward_device()
{
    auto& gd = grid.get_grid_data();
...
template<typename TF>
void Microphys_2mom_cld_warm<TF>::clear_device()
{
...
#ifdef FLOAT_SINGLE
template class Microphys_2mom_cld_warm<float>;
#else
template class Microphys_2mom_cld_warm<double>;
#endif
